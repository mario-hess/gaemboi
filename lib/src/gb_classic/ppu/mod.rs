...
use crate::{
    gb_classic::{
        bus::{OAM_END, OAM_START, VRAM_END, VRAM_START},
        cpu::interrupt::{LCD_STAT_MASK, VBLANK_MASK},
        ppu::{
            background::Background,
            lcd_control::LCD_control,
            lcd_status::{LCD_status, RenderMode},
            oam::OAM,
            tile::Tile,
            window::Window,
        },
...
impl Ppu {
    ...
    pub fn tick(&mut self, m_cycles: u8) {
        ...
        match self.lcd_status.get_mode() {
            ...
            RenderMode::OAM => {
                ...
                self.lcd_status
                    .set_mode(RenderMode::TRANSFER, &mut self.interrupts);
            ...
            RenderMode::TRANSFER => {
                ...
                self.lcd_status
                    .set_mode(RenderMode::HBLANK, &mut self.interrupts);
            ...
            RenderMode::HBLANK => {
                ...
                if self.scan_y >= LINES_Y {
                    ...
                    self.lcd_status
                        .set_mode(RenderMode::OAM, &mut self.interrupts);
                }
    ...
    fn set_lcd_control(&mut self, value: u8) {
        ...
        if !self.lcd_control.lcd_enabled() {
            ...
            self.window.reset_line_counter();
            self.set_scan_y(0);
            self.lcd_status.mode = RenderMode::HBLANK;
            self.counter = 0;
            self.enabled = false;
        }
    }
