...
pub fn ld_r_r(cpu: &mut Cpu, to: Target, from: Target) -> CycleDuration {
    let byte = cpu.registers.get_register(&from);
    cpu.registers.set_register(to, byte);
...
pub fn ld_rr_r(
    ...
    let byte = cpu.registers.get_register(&reg_target);
...
pub fn ld_rr_nn(cpu: &mut Cpu, bus: &Bus, target: Target) -> CycleDuration {
    let byte = cpu.get_nn_little_endian(bus);
    cpu.registers.set_pair(target, byte);
...
pub fn ld_r_rr(cpu: &mut Cpu, bus: &Bus, reg_target: Target, pair_target: Target) -> CycleDuration {
    ...
    cpu.registers.set_register(reg_target, byte);

    CycleDuration::Default
}

// Load the immediate 8-bit byte to the 8-bit target register
...
// byte of HL is incremented after the memory write
...
// Load to the absolute address specified by the 16-bit
// register HL, data from the 8-bit A register. The
// byte of HL is decremented after the memory write
...
// Add the signed immediate byte to SP and store the result in HL
...
// address specified by the 16-bit register HL. The byte
...
// address specified by the 16-bit immediate bytes
...
// byte to 0xFF and the least significant byte to the byte of
...
// the least significant byte to the byte of n, so the possible
...
// least significant byte to the byte of C, so the possible range
// is 0xFF00-0xFFFF
...
// Loads the immediate 16-bit byte into the stack pointer register
pub fn ld_sp_nn(cpu: &mut Cpu, bus: &Bus) -> CycleDuration {
    let byte = cpu.get_nn_little_endian(bus);
    cpu.stack.set_pointer(byte);
...
// 0xFF and the least significant byte to the byte of C,
